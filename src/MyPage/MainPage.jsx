    ...
    <>
      <div className="flex flex-col items-center justify-center bg-stone-100">
        <div className="w-full bg-current py-8 mb-4 relative">
          <div className="absolute left-10 top-2 hidden ipad:block ">
          ...
          <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 cursor-pointer ">
            {searched === false &&
              initialMeals.map((k) => (
                <div
                  key={k.idCategory}
                  className="max-w-xs shadow-2xl overflow-hidden transform  bg-white  hover:bg-black hover:text-white  hover: border-red-500 hover:border-4 transition hover:scale-125 rounded-lg "
                >
                  <img src={k.strCategoryThumb} alt={k.strCategory} />
                  <div className="px-6 py-4 text-center">
                    <div className="font-bold text-xl mb-2">{k.strCategory}</div>
                  </div>
                  <div className="px-6 py-4 text-center">
        ...
        <div className="flex justify-center mt-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 scale-y-50 cursor-pointer  focus:outline-sky-500">
            {searched === true &&
              myMeals.map((meal) => (
                <div
                  key={meal.idMeal}
                  className="max-w-xs shadow-2xl h-[390px] overflow-hidden transform  bg-white  transition hover:scale-125 rounded-lg gap-y-7  hover:bg-black hover:text-white  hover: border-red-500 hover:border-4"
                >
                  <img src={meal.strMealThumb} alt={meal.strMeal} onClick={() => showRecipes(meal)} />
                  <div className="px-6 py-4 text-center">
        ...
        {selectedMeal && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto ">
            <div className="bg-white p-6 rounded-lg shadow-lg max-w-prose hs-scroll-inside-body-modal-label cursor-pointer ">
              <h2 className="flex justify-center text-2xl font-seri font-bold mb-4">{selectedMeal.strMeal}</h2>
              <img src={selectedMeal.strMealThumb} alt={selectedMeal.strMeal} className="w-full mb-4" />
              ...
              <p className="flex justify-center">
                {" "}
                <strong>Area: </strong> <div className="ml-3">{selectedMeal.strArea}</div>
              </p>
